...
// console.log({ rootDir, nodeModulesDir, 'only-use-source': options.only-use-source });
// process.exit();

function bundle(options) {

	// console.log({ 'only-use-source': options.onlyUseSource });
	// process.exit();

  var sourceDir = options.source &&
    Path.resolve(cwd, options.source);
  var onlyUseSource = options.onlyUseSource != undefined ?
    options.onlyUseSource : 
    false;
